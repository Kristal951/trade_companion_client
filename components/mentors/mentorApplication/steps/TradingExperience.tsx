...
const TradingExperience: React.FC<{ setStep: (step: number) => void, handleBackAttempt: () => void }> = ({ setStep, handleBackAttempt }) => {
  ...
  return (
    <div className="animate-fade-in-right space-y-6">
      ...
      <div className="space-y-4">
        <h3 className="font-semibold text-primary text-xs uppercase tracking-wide">Track Record Verification</h3>

        {/* Optional Links */}
        <div>
          <label className="block text-xs font-medium text-dark-text mb-1">Verified Links (Optional)</label>
          <input
            type="text"
          ...
          />
          <p className="text-[11px] text-mid-text mt-1">Providing this significantly increases your approval chance.</p>
        </div>

        {/* Stats */}
        <div className="grid grid-cols-2 gap-4">
          <div>
            <label className="block text-xs font-medium text-dark-text mb-1">Avg. Monthly Return (%)</label>
            <input
              type="text"
          ...
          <div>
            <label className="block text-xs font-medium text-dark-text mb-1">Max Drawdown (%)</label>
            <input
              type="text"
        ...
        <div>
          <label className="block text-xs font-medium text-dark-text mb-1">Broker Trade History (Mandatory)</label>
          <FileUpload
            label={formData.tradeHistoryFile ? "File Selected" : "Upload trade history"}
      ...
      <div className="space-y-4 pt-4 border-t border-light-gray">
        <h3 className="font-semibold text-primary text-xs uppercase tracking-wide">Strategy & Methodology</h3>

        <div>
          <label className="block text-xs font-medium text-dark-text mb-1">Describe your Primary Strategy</label>
          <textarea
            rows={4}
        ...
        <div>
          <label className="block text-xs font-medium text-dark-text mb-1">Signal Generation Method</label>
          <textarea
            rows={2}
