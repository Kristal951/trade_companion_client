...
const AuthPage: React.FC<AuthPageProps> = ({ onLogin, onBack }) => {
    ...
    const handleSubmit = (e: React.FormEvent) => {
        ...
        setTimeout(() => {
            ...
            // For signup, use entered name. For login, assume a name based on email or default.
            const name = isLogin ? 'Olaniyi Peter' : formData.name; // Using mock user name for login consistency or dynamic for signup
            
            // Simple logic to mimic the previous hardcoded user if logging in with that specific email, 
    ...
    return (
        <div className="min-h-screen flex items-center justify-center bg-light-bg px-4 relative overflow-hidden">
            ...
            <button onClick={onBack} className="absolute top-6 left-6 flex items-center text-mid-text hover:text-dark-text transition-colors z-20 font-medium">
                <Icon name="arrowLeft" className="w-5 h-5 mr-2" /> Back
            </button>

            <div className="bg-light-surface p-8 rounded-2xl shadow-xl border border-light-gray w-full max-w-md z-10 relative animate-fade-in-right">
                <div className="text-center mb-8">
                    <h1 className="text-3xl font-bold text-primary mb-2">Trade Companion</h1>
                    <p className="text-mid-text">{isLogin ? 'Welcome back, trader.' : 'Start your trading journey.'}</p>
                </div>

                {error && (
                    <div className="mb-4 p-3 bg-danger/10 text-danger text-sm rounded-lg border border-danger/20 flex items-center">
                        <Icon name="danger" className="w-4 h-4 mr-2" /> {error}
                    </div>
                )}
                        ...
                        <div>
                            ...
                            <div className="relative">
                                ...
                                <input 
                                    ...
                                    className="w-full pl-10 p-3 bg-light-hover border border-light-gray rounded-lg focus:ring-2 focus:ring-primary focus:outline-none text-dark-text" 
                                />
                            </div>
                        </div>
                    )}
                ...
                <div className="mt-6 flex items-center justify-between">
                    ...
                    <span className="text-xs text-mid-text px-2 uppercase">Or continue with</span>
                    <span className="h-px bg-light-gray flex-1"></span>
                </div>
                ...
                <div className="mt-8 text-center">
                    <p className="text-sm text-mid-text">
                        ...
                        <button onClick={() => setIsLogin(!isLogin)} className="text-primary font-bold hover:underline transition-colors">
