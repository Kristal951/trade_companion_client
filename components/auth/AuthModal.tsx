...
const AuthModal: React.FC<AuthModalProps> = ({ isOpen, onClose, initialMode, onAuthSuccess }) => {
    ...
    const handleSubmit = (e: React.FormEvent) => {
        ...
        setTimeout(() => {
            setIsLoading(false);
            // Mock successful login/signup logic from the provided snippet
            // For signup, use entered name. For login, assume a name based on email or default 'Olaniyi Peter'.
            const name = isLogin ? 'Olaniyi Peter' : formData.name; 
            
            onAuthSuccess({ name: name, email: formData.email });
            onClose();
        }, 1500);
    };

    return (
        <div className="fixed inset-0 z-[60] flex items-center justify-center p-4 bg-black/60 backdrop-blur-sm animate-fade-in-right">
            {/* Modal Container */}
            <div className="bg-light-surface w-full max-w-md rounded-2xl shadow-2xl border border-light-gray overflow-hidden relative">
                
                {/* Background decoration similar to provided design */}
                ...
                {/* Close Button */}
                <button onClick={onClose} className="absolute top-4 right-4 flex items-center text-mid-text hover:text-dark-text transition-colors z-20 p-2 rounded-full hover:bg-light-hover">
