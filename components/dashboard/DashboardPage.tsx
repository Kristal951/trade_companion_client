      ...
      <button
        ...
        disabled={
          (displayName === user.name &&
            avatarPreview === (user.avatar || user.picture || user.image)) ||
          loading
        }
      >
    ...
    <div className="p-6 bg-light-surface rounded-lg border border-light-gray text-dark-text">
      ...
      <div className="p-6 bg-primary/5 rounded-xl border border-primary/10 mb-6">
        <div className="flex justify-between items-center flex-wrap gap-4">
          <div>
            <p className="text-mid-text text-sm uppercase tracking-wider font-semibold">
              Current Plan
            </p>
            ...
            <div
              ...
              className={`p-4 rounded-lg border-2 flex flex-col ${
                user.subscribedPlan === plan
                  ...
                  : "border-light-gray bg-light-hover"
              }`}
            >
              <h4 className="font-bold text-lg">{plan}</h4>
              <p className="text-2xl font-bold my-2">
                ${PLAN_COSTS[plan].price}
                <span className="text-sm font-normal text-mid-text">/mo</span>
              </p>
              <ul className="text-xs text-mid-text space-y-2 mb-4 flex-grow">
                {PLAN_FEATURES[plan].slice(0, 4).map((f, i) => (
                  <li key={i} className="flex items-start">
                    <Icon
                      name="check"
                      className="w-3 h-3 mr-1 text-success mt-0.5"
                    />
                    {f}
                  </li>
                ))}
...
const DashboardOverview: React.FC<{
  ...
  const chartEquityData = useMemo(() => {
    const data: { name: string; equity: number }[] = [
      { name: "Start", equity: initialEquity },
    ];
  ...
  const totalWins = closedTrades.filter(
  ...
  return (
    <div className="p-4 bg-light-bg min-h-[calc(100vh-64px)]">
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6 mb-8">
        ...
        <div
          onClick={() => onViewChange("ai_signals")}
          className="cursor-pointer transition-transform transform hover:scale-105"
        >
          <StatCard
...
const DashboardPage: React.FC<DashboardPageProps> = ({
  ...
  useEffect(() => {
    ...
    if (sharedMentorId) {
      const mentor = MOCK_MENTORS_LIST.find(
        (m) => m.id === parseInt(sharedMentorId)
      );
      if (mentor) {
  ...
  const renderContent = () => {
    switch (activeView) {
      ...
      case "education":
        return <EducationPage onViewContent={handleViewEducationContent} />;
      case "education_content":
        return selectedEducationArticle ? (
          <EducationContentPage
            article={selectedEducationArticle}
            onBack={handleBackToEducation}
          />
        ) : (
          <EducationPage onViewContent={handleViewEducationContent} />
        );
      case "lot_size_calculator":
        return <LotSizeCalculatorPage user={user} />;
      case "market_chart":
        return <MarketChartPage theme={theme} />;
      case "settings":
        return (
          <SettingsPage
            user={user}
      ...
      case "mentor_payouts":
        return <MentorPayoutsPage showToast={showToast} />;
      case "contact_us":
        return <ContactUsPage showToast={showToast} />;
      default:
        return (
          <DashboardOverview
            user={user}
        ...
        <header className="bg-light-surface border-b border-light-gray p-4 flex justify-between items-center flex-shrink-0">
          ...
          <div className="flex items-center space-x-4">
            <label
              ...
              className="theme-toggle-label no-print"
            >
              <input
                id="theme-toggle"
                type="checkbox"
